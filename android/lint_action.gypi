...

{
  ...
  'inputs': [
    ...
    '<(DEPTH)/chromium/build/android/gyp/lint.py',
    '<(DEPTH)/chromium/build/android/lint/suppressions.xml',
    '<(DEPTH)/chromium/build/android/AndroidManifest.xml',
    '<(lint_jar_path)',
  ],
  'action': [
    'python', '<(DEPTH)/chromium/build/android/gyp/lint.py',
    '--lint-path=<(android_sdk_root)/tools/lint',
    '--config-path=<(DEPTH)/chromium/build/android/lint/suppressions.xml',
    '--processed-config-path=<(config_path)',
    '--manifest-path=<(DEPTH)/chromium/build/android/AndroidManifest.xml',
