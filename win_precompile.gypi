...

{
  'conditions': [
    ['OS=="win" and chromium_win_pch==1', {
        'target_defaults': {
          'msvs_precompiled_header': '<(DEPTH)/chromium/build/precompile.h',
          'msvs_precompiled_source': '<(DEPTH)/chromium/build/precompile.cc',
          'sources': ['<(DEPTH)/chromium/build/precompile.cc'],
        }
      }],
  ],
}
